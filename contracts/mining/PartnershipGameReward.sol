...
contract PartnershipGameReward is Ownable {
	using SafeMath for uint256;
	using SafeERC20 for IERC20;
	// Info of each user.
	struct UserInfo {
		uint256 amount; // How many game tokens the user has provided.
		...
		//
		// Whenever a user deposits or withdraws game tokens to a pool. Here's what happens:
	...
	// check if the game token already been added or not
	...
	// Info of each user that stakes game tokens.
	mapping(uint256 => mapping(address => UserInfo)) public userInfo;
	// Total allocation poitns. Must be the sum of all allocation points in all pools.
	uint256 public totalAllocPoint = 0;

	// AAVE Special event whitelist
	...
	// admin actions event
	event LogPoolAddition(uint256 indexed pid, uint256 allocPoint, IERC20 indexed gameToken, bool withUpdate);
	event LogSetPool(uint256 indexed pid, uint256 allocPoint, IERC20 indexed gameToken, bool withUpdate);
	event LogUpdatePool(uint256 indexed pid, uint256 lastRewardBlock, IERC20 indexed gameToken, uint256 accEntropyPerShare);
	...
	modifier onlyWhitelisted() {
		require(isWhitelisted[msg.sender] == true, "LPFARM: Only whitelisted players");
		_;
	}
	...
	// Add a new game token to the pool. Can only be called by the owner.
	function add(
		uint256 _allocPoint,
		address _gameToken,
		bool _withUpdate
	) external onlyOwner {
		require(isTokenAdded[_gameToken] == false, "LPFARM: SPONSOR TOKEN ALREADY IN POOL");
		isTokenAdded[_gameToken] = true;
		if (_withUpdate) {
			massUpdatePools();
		}
		uint256 lastRewardBlock = block.number;
		totalAllocPoint = totalAllocPoint.add(_allocPoint);
		gameToken.push(IERC20(_gameToken));
		poolInfo.push(PoolInfo({ allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accEntropyPerShare: 0 }));
		getPoolID[_gameToken] = poolInfo.length.sub(1);
		emit LogPoolAddition(poolInfo.length.sub(1), _allocPoint, IERC20(_gameToken), _withUpdate);
	}
	...
	function set(
		...
		emit LogSetPool(_pid, _allocPoint, gameToken[_pid], _withUpdate);
	}
	...
	function pendingEntropy(uint256 _pid, address _user) external view validatePoolByPid(_pid) returns (uint256) {
		...
		uint256 gameTokenSupply = gameToken[_pid].balanceOf(address(this));
		if (block.number > pool.lastRewardBlock && gameTokenSupply != 0) {
			uint256 blocks = block.number.sub(pool.lastRewardBlock);
			uint256 entropyReward = blocks.mul(entropyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
			accEntropyPerShare = accEntropyPerShare.add(entropyReward.mul(1e12).div(gameTokenSupply));
		}
	...
	function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
		...
		uint256 gameTokenSupply = gameToken[_pid].balanceOf(address(this));
		if (gameTokenSupply == 0) {
			pool.lastRewardBlock = block.number;
			return;
		}
		uint256 blocks = block.number.sub(pool.lastRewardBlock);
		uint256 entropyReward = blocks.mul(entropyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
		pool.accEntropyPerShare = pool.accEntropyPerShare.add(entropyReward.mul(1e12).div(gameTokenSupply));
		pool.lastRewardBlock = block.number;
		emit LogUpdatePool(_pid, pool.lastRewardBlock, gameToken[_pid], pool.accEntropyPerShare);
	}

	// Deposit lp tokens to MasterChef for ENTROPY allocation.
	function deposit(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) onlyWhitelisted {
		PoolInfo storage pool = poolInfo[_pid];
		...
		gameToken[_pid].safeTransferFrom(address(msg.sender), address(this), _amount);
	...
	function withdraw(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) onlyWhitelisted {
		PoolInfo storage pool = poolInfo[_pid];
		UserInfo storage user = userInfo[_pid][msg.sender];
		...
		user.rewardDebt = user.amount.mul(pool.accEntropyPerShare).div(1e12);
		gameToken[_pid].safeTransfer(address(msg.sender), _amount);
		emit Withdraw(msg.sender, _pid, _amount);
	}

	// Claim mint entropy tokens
	function claim(uint256 _pid) external validatePoolByPid(_pid) onlyWhitelisted {
		PoolInfo storage pool = poolInfo[_pid];
		UserInfo storage user = userInfo[_pid][msg.sender];
		updatePool(_pid);
	...
	function emergencyWithdraw(uint256 _pid) external validatePoolByPid(_pid) onlyWhitelisted {
		UserInfo storage user = userInfo[_pid][msg.sender];
		...
		gameToken[_pid].safeTransfer(address(msg.sender), amount);
