...
import { Entropy, EntropyLiquidityFarm, EntropySponsorFarm, TestToken, TreasuryVester } from "../types";
import { v2Fixture } from "./shared/fixtures";
import { formatEther, parseEther } from "ethers/lib/utils";
import { AddressZero, MaxUint256 } from "@ethersproject/constants";

describe(" TEST: EntropyLiquidityFarm", async () => {
	...
	let liquidityFarm: EntropyLiquidityFarm;
	...
	beforeEach(async () => {
		...
		liquidityFarm = fixture.liquidityFarm;
		testToken0 = fixture.testToken0;
		testToken1 = fixture.testToken1;
		await erpToken.transfer(liquidityFarm.address, parseEther("1000"));
	});

	describe("EntropyLiquidityFarm deployed", async () => {
		it(" check variables", async () => {
			expect(await liquidityFarm.entropy()).to.be.eq(erpToken.address);
			expect(await liquidityFarm.entropyPerBlock()).to.be.eq(entropyPerBlock);
		});

		it("initial pool length", async () => {
			expect(await liquidityFarm.poolLength()).to.be.eq(0);
		});
	});
	...
	describe("test with testToken0 and testToken1", async () => {
		beforeEach(async () => {
			...
			await testToken0.connect(wallet2).approve(liquidityFarm.address, MaxUint256);
			await testToken0.connect(wallet1).approve(liquidityFarm.address, MaxUint256);
			await testToken1.connect(wallet2).approve(liquidityFarm.address, MaxUint256);
		});
		it("# add ", async () => {
			expect(await liquidityFarm.add(1, testToken0.address, false)).to.emit(liquidityFarm, "LogPoolAddition");
			expect(await liquidityFarm.poolLength()).to.be.eq(1);
			expect(await liquidityFarm.add(2, testToken1.address, false)).to.emit(liquidityFarm, "LogPoolAddition");
			expect(await liquidityFarm.poolLength()).to.be.eq(2);
		});

		it(" # deposit, pendingEntropy, claim: only wallet1 at pid 0", async () => {
			expect(await liquidityFarm.add(1, testToken0.address, true)).to.emit(liquidityFarm, "LogPoolAddition");
			expect(await liquidityFarm.add(2, testToken1.address, true)).to.emit(liquidityFarm, "LogPoolAddition");
			await liquidityFarm.connect(wallet1).deposit(0, parseEther("0.5"));
			...
			expect(await liquidityFarm.pendingEntropy(0, wallet1.address)).to.be.eq(expectedRewards);
			// claim
			await liquidityFarm.connect(wallet1).claim(0);
		...
		it(" # deposit, pendingEntropy, claim: only wallet2 at pid1", async () => {
			expect(await liquidityFarm.add(1, testToken0.address, true)).to.emit(liquidityFarm, "LogPoolAddition");
			expect(await liquidityFarm.add(2, testToken1.address, true)).to.emit(liquidityFarm, "LogPoolAddition");
			await liquidityFarm.connect(wallet2).deposit(1, parseEther("0.5"));
			const block1 = await provider.getBlockNumber();
			...
			expect(await liquidityFarm.pendingEntropy(1, wallet2.address)).to.be.eq(expectedRewards);

			// claim
			await liquidityFarm.connect(wallet2).claim(1);
			// it alse give rewards you at the block when you claim.
		...
		it(" # deposit, pendingEntropy, claim: wallet1 at pid0, wallet2 at pid1", async () => {
			...
			expect(await liquidityFarm.pendingEntropy(0, wallet1.address)).to.be.eq(expectedRewards1);
			expect(await liquidityFarm.pendingEntropy(1, wallet2.address)).to.be.eq(expectedRewards2);
		...
		it(" # deposit, pendingEntropy, claim: wallet1, wallet2 at pid0", async () => {
			...
			await provider.send("evm_mine", []);
			await provider.send("evm_mine", []);
			await liquidityFarm.connect(wallet2).deposit(0, parseEther("0.5"));
			...
			expect(await liquidityFarm.pendingEntropy(0, wallet1.address)).to.be.eq(expectedRewards1);
			expect(await liquidityFarm.pendingEntropy(0, wallet2.address)).to.be.eq(expectedRewards2);

			// claim
			await liquidityFarm.connect(wallet1).claim(0);
			await liquidityFarm.connect(wallet2).claim(0);
			// it alse give rewards you at the block when you claim.
		...
		it(" # deposit, pendingEntropy, withdraw, claim: wallet1, wallet2 at pid0, wallet1 withdraw all then wallet2 claim", async () => {
			...
			await liquidityFarm.connect(wallet1).withdraw(0, parseEther("0.5"));
			const extraRewards1 = parseEther("5").mul(1);
			// check balance
			...
			expect(await liquidityFarm.connect(wallet1).claim(0));
			expect(await erpToken.balanceOf(wallet1.address)).to.be.eq(expectedRewards1.add(extraRewards1));
			...
			it(" # deposit, pendingEntropy, emergencyWithdraw claim: only wallet1 at pid 0", async () => {
				expect(await liquidityFarm.add(1, testToken0.address, true)).to.emit(liquidityFarm, "LogPoolAddition");
				expect(await liquidityFarm.add(2, testToken1.address, true)).to.emit(liquidityFarm, "LogPoolAddition");
				await liquidityFarm.connect(wallet1).deposit(0, parseEther("1"));
				...
				expect(await liquidityFarm.pendingEntropy(0, wallet1.address)).to.be.eq(expectedRewards);
				await liquidityFarm.connect(wallet1).emergencyWithdraw(0);
				expect(await testToken0.balanceOf(wallet1.address)).to.be.eq(parseEther("1"));
